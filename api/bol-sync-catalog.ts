...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    console.log(`[bol-sync-catalog] Processing ${uniqueEans.length} unique EANs for ${customer.seller_name}`);

    let successCount = 0;
    let errorCount = 0;
    const errors: Array<{ ean: string; error: string }> = [];

    // Process in batches to avoid timeout (max 25 seconds per batch)
    const BATCH_SIZE = 20; // ~20 products with 100ms sleep = ~2 seconds per batch
    ...
    for (let i = 0; i < uniqueEans.length; i += BATCH_SIZE) {
      ...
      // Progress update
      console.log(`[bol-sync-catalog] Progress: ${Math.min(i + BATCH_SIZE, uniqueEans.length)}/${uniqueEans.length} (${successCount} success, ${errorCount} errors)`);
    }

...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { data: customer, error: customerErr } = await supabase
    .from('bol_customers')
    .select('id, seller_name, bol_client_id, bol_client_secret, active')
    .eq('id', customerId)
    .single();
  ...
  try {
    ...
    report.inventory = { status: 'ok', items: inventory.length, score: analysis.score, findings: analysis.findings, recommendations: analysis.recommendations };
  } catch (e) {
  ...
  try {
    ...
    report.orders = { status: 'ok', count: orders.length, score: analysis.score, findings: analysis.findings, recommendations: analysis.recommendations };
  } catch (e) {
